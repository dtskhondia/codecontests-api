...
def get_codeforce():  
  ...
  for contest in contests.json()['result']:
    if contest['phase']=='BEFORE':
      startTime = timezone.datetime.utcfromtimestamp(contest['startTimeSeconds'])
      endTime = timezone.datetime.utcfromtimestamp(contest['startTimeSeconds'] + contest['durationSeconds'])
      save_contest({'name':contest['name'],
                    'startTime':startTime,
                    'endTime':endTime,
                    'site':'Codeforces'})
      
def get_crawl(): # FIXME config user agent
...
def save_contest(item):
  ...
  if name and startTime and endTime and site :
    contestObj = Contest(name=name,
                         startTime=startTime if timezone.is_aware(startTime) else timezone.make_aware(startTime),
                         endTime=endTime if timezone.is_aware(endTime) else timezone.make_aware(endTime),
                         site=site)
    contestObj.save()
